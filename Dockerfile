...
RUN BUILD_DATE=20161026 \
 && apk add --no-cache ruby-bundler ruby-rdoc ruby-irb sudo \
        ruby-dev gcc g++ make cmake mariadb-dev openssh-dev imagemagick-dev \
...
RUN chown -R redmine:nobody /srv/redmine \
 ...
 && apk del ruby-dev gcc g++ make cmake mariadb-dev openssh-dev imagemagick-dev
RUN apk add --no-cache libstdc++ mariadb-client-libs tzdata imagemagick \
